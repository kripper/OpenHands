...
from openhands.events.observation import Observation, GenericObservation, BrowserOutputObservation
...
async def browse(
    ...
    if isinstance(action, BrowseURLAction):
        ...
        # action in BrowserGym: see https://github.com/ServiceNow/BrowserGym/blob/main/browsergym/core/src/browsergym/core/action/functions.py
        action_str = action.browser_actions
    else:
        raise ValueError(f'Invalid action type: {action.action}')

    try:
        # obs provided by BrowserGym: see https://github.com/ServiceNow/BrowserGym/blob/main/core/src/browsergym/core/env.py
        obs = await call_sync_from_async(browser.step, action_str)
        type = obs.get("type")
        if type == ObservationType.GENERIC:
            obs.pop("type")
            return GenericObservation('', obs)
        else:
            return BrowserOutputObservation(
                content=obs['text_content'],  # text content of the page generated in: https://github.com/ServiceNow/BrowserGym/blob/main/browsergym/core/src/browsergym/core/action/utils.py
